...
function HideStuff():void {
  let pan = $.GetContextPanel().GetParent().GetParent()
  let center_block = pan.FindChildTraverse("center_block")
  if (center_block) { hidden = true}
  ...
  for (let p of pan.FindChildrenWithClassTraverse("AbilityInsetShadowLeft")) {
    p.style.visibility = "collapse"
  }

  let panel =  pan.FindChildTraverse("AbilitiesAndStatBranch")
